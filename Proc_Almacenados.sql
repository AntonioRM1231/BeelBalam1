...
CREATE PROCEDURE VALIDAR_INICIOSESION(@user VARCHAR(25))
AS
BEGIN
	SELECT CONTRASENIA_U FROM USUARIO WHERE NOMBRE_U = @user;
END

EXECUTE VALIDAR_INICIOSESION 'pollito'

CREATE PROCEDURE GET_USERDATA(@user VARCHAR(25))
AS
BEGIN
	SELECT *FROM USUARIO WHERE NOMBRE_U = @user;
END

EXECUTE GET_USERDATA 'pollito'

DROP PROCEDURE EDIT_U_SIMPLE
CREATE PROCEDURE EDIT_U_SIMPLE(
@u_nombre VARCHAR (25),
@u_newNombre VARCHAR (25),
@u_newContraseña VARCHAR (20),
@u_newCorreo VARCHAR (35),
...
BEGIN
	IF EXISTS (SELECT NOMBRE_U FROM USUARIO WHERE NOMBRE_U = @u_newNombre)
		IF(@u_nombre = @u_newNombre)
			BEGIN SELECT 'MISMO NOMBRE'; END
		ELSE
			BEGIN SELECT 'ERROR, SE REPITE USAURIO'; END
	ELSE 
		BEGIN 
			UPDATE USUARIO SET NOMBRE_U = @u_newNombre WHERE NOMBRE_U = @u_nombre;
			BEGIN SELECT 'EXITO'; END
		END
END

EXECUTE EDIT_U_SIMPLE 'juli','pivo',NULL,NULL,NULL
